...
- [ ] Task 1: Update `InsightCard.tsx` headline and key metric styling (AC: #1, #3)
  - [ ] 1.1 Change headline `<h3>` from `text-base font-semibold` to `text-lg font-bold leading-snug` (make it visually dominant)
  - [ ] 1.2 Change key metric `<p>` from `text-2xl font-bold text-primary` to `text-base font-medium text-muted-foreground` (make it supporting)
  - [ ] 1.3 Verify visual hierarchy: headline should be the first thing the eye lands on

- [ ] Task 2: Refine Education Agent prompt for concise key metrics (AC: #2)
  - [ ] 2.1 In `backend/app/agents/education/prompts.py`, update all 4 prompt templates
  - [ ] 2.2 Change `key_metric` description from `"The key number (e.g., '₴4,200 on food this month')"` to `"A short metric, max 30 chars (e.g., '₴4,200 on food'). No compound expressions or percentages with comparisons."`
  - [ ] 2.3 Apply equivalent change to Ukrainian prompts: `"Коротке число, макс 30 символів (наприклад, '₴4 200 на їжу'). Без складних виразів чи порівнянь."`

- [ ] Task 3: Tests (AC: #1–#4)
  - [ ] 3.1 Update `InsightCard` test: verify headline renders with `text-lg font-bold` classes
  - [ ] 3.2 Update `InsightCard` test: verify key metric renders with `text-base font-medium text-muted-foreground` classes
  - [ ] 3.3 Verify all existing 273 backend + 184 frontend tests continue to pass
