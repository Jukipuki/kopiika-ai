...
- `chat_messages` — `id` UUID PK, `session_id` UUID FK (cascade delete), `role` text CHECK (`user` | `assistant` | `system` | `tool`) — `tool` added by Story 10.4c migration [`ca1c04c7b2e9`](../../backend/alembic/versions/ca1c04c7b2e9_add_chat_message_role_tool.py) for per-tool-call forensic rows, `content` text, `redaction_flags` JSONB (shape: `{"pii_types": ["email" | "iban" | "card" | ...], "filter_source": "input" | "output" | "input_validator" | "canary_detector" | "tool_dispatcher"}` — `input_validator` + `canary_detector` added by Story 10.4b, `tool_dispatcher` added by Story 10.4c), `guardrail_action` text CHECK (`none` | `blocked` | `modified`), `created_at` timestamptz
