...
- `chat_messages` — `id` UUID PK, `session_id` UUID FK (cascade delete), `role` text CHECK (`user` | `assistant` | `system`), `content` text, `redaction_flags` JSONB (shape: `{"pii_types": ["email" | "iban" | "card" | ...], "filter_source": "input" | "output" | "input_validator" | "canary_detector"}` — `input_validator` + `canary_detector` added by Story 10.4b for the pre-invoke / post-invoke defense layers), `guardrail_action` text CHECK (`none` | `blocked` | `modified`), `created_at` timestamptz
